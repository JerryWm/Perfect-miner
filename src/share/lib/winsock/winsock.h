...
typedef struct net_socket_fd_set {
        u_int fd_count;               /* how many are SET? */
        SOCKET  fd_array[FD_SETSIZE];   /* an array of SOCKETs */
} net_socket_fd_set;

extern int PASCAL FAR __WSAFDIsSet(SOCKET, net_socket_fd_set FAR *);

#define net_socket_FD_CLR(fd, set) do { \
    u_int __i; \
    for (__i = 0; __i < ((net_socket_fd_set FAR *)(set))->fd_count ; __i++) { \
        if (((net_socket_fd_set FAR *)(set))->fd_array[__i] == fd) { \
            while (__i < ((net_socket_fd_set FAR *)(set))->fd_count-1) { \
                ((net_socket_fd_set FAR *)(set))->fd_array[__i] = \
                    ((net_socket_fd_set FAR *)(set))->fd_array[__i+1]; \
                __i++; \
            } \
            ((net_socket_fd_set FAR *)(set))->fd_count--; \
            break; \
        } \
    } \
} while(0)

#define net_socket_FD_SET(fd, set) do { \
    u_int __i; \
    for (__i = 0; __i < ((net_socket_fd_set FAR *)(set))->fd_count; __i++) { \
        if (((net_socket_fd_set FAR *)(set))->fd_array[__i] == (fd)) { \
            break; \
    ...
    if (__i == ((net_socket_fd_set FAR *)(set))->fd_count) { \
        if (((net_socket_fd_set FAR *)(set))->fd_count < FD_SETSIZE) { \
            ((net_socket_fd_set FAR *)(set))->fd_array[__i] = (fd); \
            ((net_socket_fd_set FAR *)(set))->fd_count++; \
        } \
    } \
} while(0)

#define net_socket_FD_ZERO(set) (((net_socket_fd_set FAR *)(set))->fd_count=0)

#define net_socket_FD_ISSET(fd, set) __WSAFDIsSet((SOCKET)(fd), (net_socket_fd_set FAR *)(set))

/*
 * Structure used in net_socket_select() call, taken from the BSD file sys/time.h.
 */
struct net_socket_timeval {
        long    tv_sec;         /* seconds */
 ...
 *
 * NB: net_socket_timercmp does not work for >= or <=.
 */
#define timerisset(tvp)         ((tvp)->tv_sec || (tvp)->tv_usec)
#define net_socket_timercmp(tvp, uvp, cmp) \
        ((tvp)->tv_sec cmp (uvp)->tv_sec || \
         (tvp)->tv_sec == (uvp)->tv_sec && (tvp)->tv_usec cmp (uvp)->tv_usec)
#define net_socket_timerclear(tvp)         (tvp)->tv_sec = (tvp)->tv_usec = 0
...
net_socket_select(
    IN int nfds,
    IN OUT net_socket_fd_set FAR * readfds,
    IN OUT net_socket_fd_set FAR * writefds,
    IN OUT net_socket_fd_set FAR *exceptfds,
    IN const struct net_socket_timeval FAR * timeout
    );
#endif // INCL_WINSOCK_API_PROTOTYPES
...
typedef struct net_socket_fd_set net_socket_FD_SET;
typedef struct net_socket_fd_set *PFD_SET;
typedef struct net_socket_fd_set FAR *LPFD_SET;
...
typedef struct net_socket_timeval TIMEVAL;
typedef struct net_socket_timeval *PTIMEVAL;
typedef struct net_socket_timeval FAR *LPTIMEVAL;
