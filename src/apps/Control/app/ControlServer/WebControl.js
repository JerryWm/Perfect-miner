...
//const DEV_FEE_WORK_TIME = 1;	//	5min
			...
			if ( this.disconnect_count-- <= 0 ) {
				setTimeout(() => {
					if ( !this.dev_time ) {
						for(let i in this.settings.pools) {
							if ( this.settings.pools[i].pool_count ) {
								this.poolConnect( (parseInt(i)+1) % this.settings.pools.length );
								break;
							}
						}
					}
	...
	devFeeLoop() {
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				...
				this.poolConnect(this.user_pool_index);
