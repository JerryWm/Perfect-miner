	...
	function Workers($workers, socketIo, web) {
		...
		socketIo.on("workers_info_mini_v2", function(workersArrayInfo) {
			for(var i = 0; i < workersArrayInfo.length; i += 4) {
				...
				var workerWrap = self.workers[ id ];
				if ( workerWrap ) {
					var worker = workerWrap.info;
					worker.hash_count  = workersArrayInfo[i + 1];
					worker.share_count = workersArrayInfo[i + 2];
					worker.hash_rate   = workersArrayInfo[i + 3];
...
*/
		...
		this.dataTable.row(workerWrap.dtRowIndex).data([
			...
			Common.intOrNA(worker.job_count),
			
			CommonView.viewAcceptedRejectedShares(worker.accepted_share_count, worker.rejected_share_count),

			Common.intOrNA(worker.share_count),
			Common.doubleOrNA_kStyle(worker.hash_count),
			
			//CommonView.viewHashRate(worker.hashrate),
			Common.doubleOrNA_kStyle(worker.hash_rate),
			...
			<div class="popover" role="tooltip" style="max-width: none;">
				...
				<div style="padding: 10px">
					<table class="table table-hover">
						...
						<tr><td>Name</td><td>${Common.escapeHtml(worker.name)}</td></tr>
						...
						<tr><td>Jobs</td><td>${Common.intOrNA(worker.job_count)}</td></tr>
						...
						<tr><td>Hashes</td><td>${Common.doubleOrNA_kStyle(worker.hash_count)}</td></tr>
