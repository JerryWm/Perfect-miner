...
const WorkersServer = require("./WorkersServer");
...
class WrapperPool {
	...
	submitShare(share) {
		if ( !this.job_id || !share ) {
			return;
		}
		
		if ( share.job_id === this.job_id ) {
...
class StratumProxy {
	constructor(options, pool_connect_info, events, logger) {
		...
		this.events.on("workers:server:worker:connect"         , this.workerConnect      .bind(this));
		this.events.on("workers:server:worker:disconnect"      , this.workerDisconnect   .bind(this));
		this.events.on("workers:server:worker:app:share"       , this.workerShare        .bind(this));
		this.events.on("workers:server:worker:app:options"     , this.workerOptions      .bind(this));
		this.events.on("workers:server:worker:app:workers_info", this.workerWorkersInfo  .bind(this));
	...
	startStratumServers() {
		...
		for(let svOptions of this.options.servers) {
			list[ (new WorkersServer(svOptions, this.events, this.logger)).id ] = svOptions;
		}
	}
